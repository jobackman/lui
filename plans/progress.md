...
- Simplified tag system from 91→17 tags. Removed redundancies (dmg→damage, cd/cds/cdm→cooldowns, mythic/m+/mythic+→mythic, 8 resource types→resource, deprecated weakaura tags, etc). New tags: Combat (combat/damage/healing/meter), UI (ui/hud/frames/nameplates), Resources (resource/cooldowns), Content (raid/mythic), Utility (map/quest/misc). Made category field required (not optional) in AddonCategory interface. Updated all 15 addon files w/ simplified tags. Updated AGENTS.md w/ new tag reference. Fixed command-bar tests for new tags. Build+tests pass (32). Cleaner data model, better UX.
