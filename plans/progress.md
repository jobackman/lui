...
- Refactored button classes using class-variance-authority. Installed cva package. Created glassButtonVariants in utils.ts w/ 3 variants: card (hover-reveal animation), carousel (nav buttons), action (detail page buttons). Refactored ExportCard+AddonDetailPage to use cva variants w/ cn utility for composability. Cleaner code, maintainable variants. Build+tests pass.
