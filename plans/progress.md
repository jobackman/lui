...
- Changed background ripple to fixed positioning w/ large height and full viewport width. Uses fixed left-0 top-0 w/100vw, 3x viewport height (min 3000px). Moved outside relative container to prevent left cutoff. Grid cells wrapped in pointer-events-auto for interactivity. Scrolls naturally, gradients align properly. Build+tests pass.
