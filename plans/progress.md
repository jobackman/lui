...
- Added smooth zoom animations on image hover. ExportCard+AddonDetailPage carousel images scale to 105% on hover w/ 500ms ease-out transition. Applied overflow-hidden to image containers preventing layout break. motion-reduce:transition-none respects accessibility. Auto-pause carousel functionality unaffected. Maximize2 icon overlay on detail page carousel. Build+tests pass (32). PRD complete.
