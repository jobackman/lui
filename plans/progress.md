...
- Implemented View Transition API for route navigation. Created useViewTransition hook w/ browser support detection+fallback. Added view-transition-name to cards (unique per addon ID) via refs. Intercept Link clicks in HomePage, AddonDetailPage, CommandBar to trigger transitions. Navigate home→detail, detail→home, Escape key, command bar all use transitions. CSS respects prefers-reduced-motion. Fixed ref-based approach for proper browser support. Build+tests pass.
