...
- Implemented fixed hero header on addon detail page. Sticky hero at top (300-500px responsive height). Background carousel w/ image crossfade (duration-700). Dark gradient overlay (from-black/40 to-black/90). Title/desc/timestamp/tags overlaid on hero w/ text shadows. Action buttons on hero w/ glass-strong styling. Back button top-left w/ glass effect. Carousel controls (prev/next arrows, dots) on hero. Content card below contains only setup instructions. Keyboard nav (arrows, Escape) maintained. Build+tests pass.
